...
impl Plugin for GarageMenuPlugin {
    fn build(&self, app: &mut App) {
        // Enter
        app.add_system_set(SystemSet::on_enter(MenuState::Garage).with_system(ui::show));

        // Update
        app.add_system_set(SystemSet::on_update(MenuState::Garage).with_system(on_update));

        // Exit
        app.add_system_set(SystemSet::on_exit(MenuState::Garage).with_system(ui::hide));
    }
}
